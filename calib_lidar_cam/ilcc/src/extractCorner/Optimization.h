                    ...
                    T* residuals) const
    {
        ...
        // out of board
            ...
            if(i<T(0))
                ...
                residuals[0] = i - T(m_board_size(0));
            if(j<T(0))
                residuals[0] = residuals[0] - j;
            else
                residuals[0] = residuals[0] + j - T(m_board_size(1));
        }
...
public:
    ...
    Eigen::Isometry3d solvePose3d2dError(std::vector<cv::Point3d> pts3d,
                                         std::vector<cv::Point2d> pts2d,
                                         cv::Mat K);

    Eigen::Vector3d get_theta_t(std::vector<Eigen::Vector2d> point2d, std::vector<int> intensitys);
